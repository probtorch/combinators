...
def maybe_sample(
    ...
    sample_shape: Union[Tuple[int], None, tuple],
    reparameterized: Optional[bool] = None,
) -> Callable[[D.Distribution, str], Tuple[Tensor, Provenance, bool]]:
    def curried(dist: D.Distribution, name: str) -> Tuple[Tensor, Provenance, bool]:
        _reparameterized = (
            reparameterized if reparameterized is not None else dist.has_rsample
        )
        if trace is not None and name in trace:
            ...
            return (
                dist.rsample() if _reparameterized else dist.sample(),
                Provenance.SAMPLED,
                _reparameterized,
            )
        else:
            return (
                dist.rsample(sample_shape)
                if _reparameterized
                else dist.sample(sample_shape),
                Provenance.SAMPLED,
                _reparameterized,
            )
...
def valeq(
    t1: Trace,
    t2: Trace,
    nodes: Union[Dict[str, Any], List[Tuple[str, str]], None] = None,
    ...
    if not all(name in t1 and name in t2 for name in _nodes):
        ...
        cmpr_string = (
            "are not in {" + ", ".join(_nodes) + "}"
            if nodes is not None
            else "are not equal"
        )
        raise Exception(
            "trace's keys {}:\n  trace 1: {}\n  trace 2: {}".format(
                cmpr_string, str_t1nodes, str_t2nodes
            )
        )
    ...
    if len(invalid) > 0:
        ...
        raise Exception(
            "RV nodes {} are not equal in:\n  trace 1: {}\n  trace 2: {}".format(
                str_nodes, set(t1.keys()), set(t2.keys())
            )
        )
...
def showall(
    ...
    return (
        ...
        + delim.join(
            [
                ("{:>" + str(mlen) + "}-➢{}").format(
                    k, showRV(v, args=args, dists=dists)
                )
                for k, v in items
            ]
        )
