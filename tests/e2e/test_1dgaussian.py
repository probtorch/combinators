...
class Gaussian1d(Program):
    ...
    def model(self, trace):
        return trace.normal(loc=self.loc, scale=1, name=self.name)

@typechecked
class Kernel(Kernel):
    def __init__(self, num_hidden, name, ix=None):
        super().__init__()
        self.net = Net(dim_in=2, dim_hidden=num_hidden, dim_out=2)
        self.name = name if ix is None else name + "_" + str(ix)

    def apply_kernel(self, trace, cond_trace, obs):
        return trace.normal(loc=self.net(obs), scale=1, name=self.name)
...
def test_Gaussian_1step():
    ...
    proposal,   target     = Gaussian1d(loc=1, name="q"), Gaussian1d(loc=1, name="p")
    fwd_kernel, rev_kernel = Kernel(4, "fwd"),   Kernel(4, "rev")
    ...
    with trange(num_steps) as bar:
        for i in bar:
            # lvs, ps, qs = [], [], []
            # p_prev, q_prev = probtorch.Trace(), probtorch.Trace()
            # q_prev.append(proposal(sample_shape=(1, L)), name='z_0')
            # p_prev.append(targets[0](value=q_prev['z_0']), name='z_0')
            # ps.append(p_prev)
            # qs.append(q_prev)
            # lw = log_inc_weights(p_prev, q_prev)
            # lvs.append(lw)
            #=====================================
            ...
            q_ext_trace, _ = Forward(fwd_kernel, proposal)()()
            # p_tar_trace, _ = target()
            p_ext_trace, _ = Reverse(target, rev_kernel)()()
            log_weights    = p_ext_trace.log_joint(batch_dim=None) - q_ext_trace.log_joint(batch_dim=None)
            #=====================================
            log_weights.backward()
            optimizer.step()
            optimizer.zero_grad()
            bar.set_postfix(dict(loss=log_weights.sum()))
