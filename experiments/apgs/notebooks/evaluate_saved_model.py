...
model_version = "apg-timesteps=%d-objects=%d-sweeps=%d-samples=%d%s" % (
    timesteps,
    num_objects,
    num_sweeps,
    sample_size,
    "-with_opt" if use_markov_blanket else "",
)
models = init_models(
...
out, frames = (
    apg({"frames": frames_expand}, sample_dims=0, batch_dim=1, reparameterized=True),
    frames,
)
...
def get_samples(out, sweeps, T):
    ...
    for t in range(T):
        z_where_vals[:, :, t, :, :] = (
            out.trace["z_where_%d_%d" % (t, sweeps)].value.detach().cpu()
        )
