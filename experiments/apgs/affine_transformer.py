...
class Affine_Transformer:
    def __init__(self, frame_pixels, digit_pixels, device):
        ...
        self.translation_dtof = (
            self.frame_pixels - self.digit_pixels
        ) / self.digit_pixels
        self.translation_ftod = (
            self.frame_pixels - self.digit_pixels
        ) / self.frame_pixels
        self.scale_dtof = torch.tensor(
            [
                [self.frame_pixels / self.digit_pixels, 0],
                [0, self.frame_pixels / self.digit_pixels],
            ],
            device=device,
            dtype=torch.float,
        )
        self.scale_ftod = torch.tensor(
            [
                [self.digit_pixels / self.frame_pixels, 0],
                [0, self.digit_pixels / self.frame_pixels],
            ],
            device=device,
            dtype=torch.float,
        )
    ...
    def digit_to_frame(self, digit, z_where):
        ...
        affine_p2 = (
            z_where.unsqueeze(-1) * self.translation_dtof
        )  ## S * B * T * K * 2 * 1
        affine_p2[:, :, :, :, 0, :] = (
            -1 * affine_p2[:, :, :, :, 0, :]
        )  ## flip the x-axis due to grid function
        grid = affine_grid(
            torch.cat((affine_p1, affine_p2), -1).view(S * B * T * K, 2, 3),
            torch.Size((S * B * T * K, 1, self.frame_pixels, self.frame_pixels)),
            align_corners=True,
        )
        frames = grid_sample(
            digit.unsqueeze(2)
            .repeat(1, 1, T, 1, 1, 1)
            .view(S * B * T * K, self.digit_pixels, self.digit_pixels)
            .unsqueeze(1),
            grid,
            mode="nearest",
            align_corners=True,
    ...
    def frame_to_digit(self, frames, z_where):
        ...
        affine_p2 = (
            z_where.unsqueeze(-1) * self.translation_ftod
        )  ## S * B * T * K * 2 * 1
        affine_p2[:, :, :, :, 1, :] = (
            -1 * affine_p2[:, :, :, :, 1, :]
        )  ## flip the y-axis due to grid function
        grid = affine_grid(
            torch.cat((affine_p1, affine_p2), -1).view(S * B * T * K, 2, 3),
            torch.Size((S * B * T * K, 1, self.digit_pixels, self.digit_pixels)),
            align_corners=True,
        )
        digit = grid_sample(
            frames.unsqueeze(-3)
            .repeat(1, 1, 1, K, 1, 1)
            .view(S * B * T * K, self.frame_pixels, self.frame_pixels)
            .unsqueeze(1),
            grid,
            mode="nearest",
            align_corners=True,
