...
class Sim_BShape:
    ...
    def load_resize_shapes(self):
        ...
        for p in shape_paths:
            ...
            if self.padding_size > 0 and isinstance(self.padding_size, int):
                ...
                padded_img[
                    self.padding_size : self.padding_size + self.shape_size,
                    self.padding_size : self.padding_size + self.shape_size,
                ] = resized_img
            else:
    ...
    def sim_trajectories(self, num_tjs, save_flag=False):
        ...
        while True:
            if (
                ((x0[0] - x0[1]) ** 2).sum() > a2
                and ((x0[2] - x0[1]) ** 2).sum() > a2
                and ((x0[0] - x0[2]) ** 2).sum() > a2
            ):
                break
            x0 = Uniform(-1, 1).sample((num_tjs, 2))
    ...
    def sim_one_sequence(self, objects):
        ...
        for k in range(self.num_objects):
            object_k = objects[k]
            S = torch.Tensor([[s_factor, 0], [0, s_factor]]).repeat(
                self.timesteps, 1, 1
            )
            Thetas = torch.cat((S, Xs[k].unsqueeze(-1) * t_factor), -1)
            grid = affine_grid(
                Thetas,
                torch.Size((self.timesteps, 1, self.frame_size, self.frame_size)),
                align_corners=False,
            )
            bshape.append(
                grid_sample(
                    object_k.repeat(self.timesteps, 1, 1).unsqueeze(1),
                    grid,
                    mode="nearest",
                    align_corners=False,
                )
    ...
    def sim_save_data(self, num_seqs, PATH):
        ...
        while num_seqs_left > 0:
            ...
            assert seqs.shape == (
                ...
                self.frame_size,
                self.frame_size,
            ), "ERROR! unexpected chunk shape."
            incremental_PATH = PATH + "seq-timesteps=%d-objects=%d-p%d" % (
                self.timesteps,
                self.num_objects,
                counter,
            )
            ...
            time_end = time.time()
            print(
                "(%ds) Simulated %d sequences, saved to '%s', %d sequences left."
                % (
                    (time_end - time_start),
                    num_this_round,
                    incremental_PATH,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--timesteps", default=10, type=int, help="number of video frames in one video"
    )
    parser.add_argument(
        "--num_objects", default=3, type=int, help="number of objects in one video"
    )
    parser.add_argument(
        "--dv", default=0.1, type=float, help="constant velocity of the digits"
    )
    parser.add_argument(
        "--frame_size", default=96, type=int, help="squared size of the canvas"
    )
    ...
    parser.add_argument(
        ...
        default=1000,
        type=int,
        help="number of sqeuences that are stored in one single file (for the purpose of memory saving)",
    )
    args = parser.parse_args()
    simulator = Sim_BShape(
        args.timesteps,
        args.num_objects,
        args.frame_size,
        args.shape_size,
        args.padding_size,
        eval(args.blacklist_shapes),
        args.shape_dir,
        args.dv,
        args.chunk_size,
    )
    print(
        "simulating with %d objects, %d timesteps, dv=%.1f"
        % (args.num_objects, args.timesteps, args.dv)
    )
