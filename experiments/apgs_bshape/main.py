...
def train(
    ...
    epochs = range(1) if is_smoketest else trange(num_epochs, desc='Epochs')
    tdata_paths = [(0, data_paths[0])] if is_smoketest else tenumerate(data_paths, desc=f'data set')
    ...
    for epoch in epochs:
        ...
        for group, data_path in tdata_paths:
            ...
            with trange(3 if is_smoketest else num_batches, desc=f'batch') as batches:
                for b in batches:
                    optimizer.zero_grad()
                    frames = data[seq_indices[b*batch_size : (b+1)*batch_size]].repeat(sample_size, 1, 1, 1, 1)
                    state = objective(models, AT, frames, K, result_flags, num_sweeps, resampler, shape_mean)

                    state.loss['phi'].sum().backward(retain_graph=True)
                    state.loss['theta'].sum().backward()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', default=f'{git_root()}/data/bshape/')
    ...
    num_sweeps = 1
    sample_size = int(args.budget / num_sweeps)
    debug.seed(0)
    ...
    if num_sweeps == 1: ## rws method
        ...
        model_version = 'apg-bshape-num_objects=%s-num_sweeps=%s-num_samples=%s' % (args.num_objects, num_sweeps, sample_size)
    else:
        raise ValueError
