...
keyspec = namedtuple('keyspec', "resample optimize_path num_targets objective num_iters sample_budget")
def from_file_prefix(k):
    ...
    return keyspec(resampled,optimize_path,num_targets,objective,num_iters,sample_budget)
    ...
    for resample in [True, False]:
        for optimize_path in [True, False]:
            for objective_name in ['nvo_avo', 'nvo_rkl']:
                for K in [2, 4, 6, 8]:
                    yield get_file_prefix(resample, optimize_path, objective_name, S, K, I)
...
def tuple_to_report(losses, esss, lZhs):
    ...
    return dict(loss=loss.detach().mean(0), ess=ess.mean(0), lZh=lZh.mean(0), num_seeds=num_seeds)
...
def main():
    ...
    for k, ts in stats.items():
        ...
        if runtype[-2] == "r" or runtype[-1] == "r":
            resampled.append((k, int(kk[1]), runtype[-1] == "s", nvo_avo if objective == 'nvo' else nvo_rkl))
        else:
        ...
        for (f, model, objective) in tqdm(smodel, total=len(smodel)):
            for i in range(startseed, startseed + nruns):
                if fast:
                    ...
                    torch.manual_seed(i)
                    loss, ess, lZh = rerun_test(model, objective, filename=f.split(".")[0])
    ...
    for key, metrics in all_stats.items():
        ...
        table.append(('avo' if objective == 'avo' else runtype, str(metrics['num_seeds']), kk[1],
    ...
    # models = load_models("./sync/weights")


if __name__ == '__main__':
    main()

# output resampled models rerun with seed = 0
_ = '''
method |  # | num_targets |      ess |    lZh
------------------------------------------------
  nvis | 10 |           2 | 426.5490 | 1.8767
  nvis | 10 |           4 | 318.3964 | 2.0445
  nvis | 10 |           6 | 403.1296 | 2.0682
  nvis | 10 |           8 | 413.0998 | 2.0707
------------------------------------------------
   avo | 10 |           2 | 426.3823 | 1.8760
   avo | 10 |           4 | 291.2084 | 1.9821
   avo | 10 |           6 | 284.9513 | 2.0477
   avo | 10 |           8 | 294.6565 | 2.0566
------------------------------------------------
   nvi | 10 |           2 | 426.5490 | 1.8767
   nvi | 10 |           4 | 341.2098 | 1.9878
   nvi | 10 |           6 | 307.5547 | 2.0609
   nvi | 10 |           8 | 319.0592 | 2.0677
------------------------------------------------
 nvirs | 10 |           2 | 423.6697 | 1.8765
 nvirs | 10 |           4 | 319.0118 | 2.0478
 nvirs | 10 |           6 | 410.9373 | 2.0713
 nvirs | 10 |           8 | 415.6058 | 2.0716
------------------------------------------------
  nvir | 10 |           2 | 423.6697 | 1.8765
  nvir | 10 |           4 | 354.3758 | 1.9796
  nvir | 10 |           6 | 333.6036 | 2.0414
  nvir | 10 |           8 | 337.1406 | 2.0642
'''

# output resampled models rerun with seed = 1
_ = '''
method |  # | num_targets |      ess |    lZh
------------------------------------------------
  nvis | 10 |           2 | 426.5490 | 1.8767
...
------------------------------------------------
   ...
   nvi | 10 |           8 | 319.0592 | 2.0677
------------------------------------------------
 nvirs | 10 |           2 | 418.4496 | 1.8746
 nvirs | 10 |           4 | 321.1141 | 2.0732
 nvirs | 10 |           6 | 408.7328 | 2.0747
 nvirs | 10 |           8 | 410.0186 | 2.0736
------------------------------------------------
  nvir | 10 |           2 | 418.4496 | 1.8746
  nvir | 10 |           4 | 349.6178 | 1.9890
  nvir | 10 |           6 | 311.2580 | 2.0613
  nvir | 10 |           8 | 326.6396 | 2.0730
'''
