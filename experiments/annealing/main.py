...
def check_weights_zero(forwards, reverses):
    for forward, reverse in zip(forwards, reverses):
        assert (forward.net.map_cov[0].weight == 0.0).all()
        assert (reverse.net.map_cov[0].weight == 0.0).all()
        assert (forward.net.map_mu[0].weight == 0.0).all()
        assert (reverse.net.map_mu[0].weight == 0.0).all()
        assert (forward.net.map_mu[0].bias == 0.0).all()
        assert (reverse.net.map_mu[0].bias == 0.0).all()
...
if __name__ == "__main__":
    ...
    q = nvi_declarative(*model, objective, resample=resample)
    S = 288
    iterations = args.iterations
    ...
    q = nvi_declarative(*model, objective, resample=False)
